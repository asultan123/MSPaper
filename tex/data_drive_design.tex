...
by dividing it into two distinct parts. The first part involves the
...
Note this part of the exploration limits the unroll factor
exploration to only the kernel loops. The second part of the dataflow design
...
knowledge of the actual hardware architecture. As such, it is addressed through
the use of a tool named TEMPO in the subsequent chapter of this thesis.


In addition to identifying an optimal dataflow for the common
...
After identifying the optimal loop unroll targets and factors for the kernel
loops, the next step is to analyze the reuse behavior of the data elements in
the selected dataflow. This can be accomplished by applying the method described
in \cite{meeus}. Once the reuse behavior has been determined, the hardware
implementation taxonomy from \cite{maestro} can be employed to determine the
most suitable hardware implementation for the specific reuse behavior found in
the selected dataflow.


This chapter is broken down as follows we will first prune the dataflow design
space dimensions in \autoref{chap:dda:dataflow_dse:pruning} by determining the
appropriate loop unroll targets and loop unroll factors using insights acquired
from \ac{CIGAR} a convolutional network analysis tool discussed in
\autoref{chap:dda:dataflow_dse:pruning:cigar} along with an overview of the TIMM
library of networks. Then, given the complexity of exploring loop unroll factors
...
explore the the aforementioned design space dimensions to produce utilization
optimized accelerator template configurations. An exploration of the overlap
between (1, 1) convolutions and GEMM to extend support to arbitrary convolutions
is discussed in \autoref{chap:dda:dataflow_dse:GEMM_mode}. Upon identifying the
loop unroll targets and loop unroll factors for kernel loops this chapter
explores the hardware implementation space based on the reuse behavior of the
chosen dataflow in \autoref{chap:dda:hw_dse}. In \autoref{chap:dda:hw_dse:final}
the final HERO architectural template is presented.
...
directly are executed on the accelerator and an GEMM mode where kernels that
...
explanation of GEMM mode is given in
\autoref{chap:dda:dataflow_dse:GEMM_mode}. 

Note that convolution layers with
non (1, 1) strides are executed under GEMM mode. Supporting convolution layers
with non (1, 1) strides directly is left as part of future work. From
\autoref{fig:kernel_stats:strides} (1, 1) strides are present in ~95\% of
...
In summary, we propose to take advantage of the high weight reuse observed in
many networks by using a weight stationary approach. Weight stationary sets the
loop unroll targets to the F, C, KY, and KC loops, with the assumption that the
stride size is limited to (1, 1) due to its prevalence. By unrolling the F, C
loops we can maximize weight reuse as well as parallelize layers that have been
converted to GEMM. 

Additionally, we also need to unroll the KY and KX loops to avoid having to
store weights on-chip at a large scale, as it is an inefficient use of memory.
Given the high frequency of (1, 1) kernel sizes, we need to unroll the KY, KX
loops to this minimum value. Additionally, (1, 1) kernel sizes form the basis of
GEMM mode which strengthens the argument for direct support (1, 1) kernels.
Additionally, by considering the frequency of occurrence and the number of MACs,
we also unroll the KY and KX loops to (3, 3).
...
To allow HERO to run arbitrary convolution layers, the overlap between
convolutions with a kernel size of (1,1) and general matrix multiplication will
be examined in this section. This overlap enables HERO to support arbitrary
convolution layers by first interpreting them as GEMM operations then
reinterpreting them against as (1, 1) convolutions which, from the previous
section, are one of the kernel sizes directly supported by HERO. This
operational mode is called GEMM/ Indirect mode.  
GEMM mode extends support to convolution layers with kernel sizes that are not
directly supported by an convolution accelerator. It does this by establishing a
layer equivalence relationship between unsupported layers and supported layers.
...
\cite{cafe_con_troll} and in the \autoref{chap:background:intro}. This approach
...
Whether or not a convolution layer's kernel size is supported directly has an effect on the assumed
...
\subsection{Accelerator Spatial Axis mapping}
...
where a subset of possible kernel sizes are supported and GEMM/ Indirect mode
where all other kernel sizes and strides are supported via the lowering/ lifting
based approach. 
