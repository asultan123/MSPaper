...
\autoref{chap:dda:dataflow_dse:GEMM_mode:conv_gemm_equiv:proof}, the choice
...
\autoref{chap:dda:dataflow_dse:GEMM_mode}. Note that convolution layers with
...
\section{Enabling GEMM mode through layer equivalence}
\label{chap:dda:dataflow_dse:GEMM_mode}

GEMM mode extends support to convolution layers with kernel sizes that are
...
directly. In
\autoref{chap:dda:dataflow_dse:GEMM_mode:conv_gemm_equiv:proof}, a proof for
the equivalence between GEMM and (1, 1) Convolutions is given. Additionally, in
\autoref{chap:dda:dataflow_dse:GEMM_mode:layer_equivelence} the
