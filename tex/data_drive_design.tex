...
this chapter is defined as data access and communication patterns within the accelerator. 
Hardware implementation is defined as the on-chip communication
...
operations concurrently across different loop boundaries.
...
A summary of the different elements and their associated Reuse and Communication
Types is presented in \autoref{tab:summary_of_reuse_behavior}. Applying the taxonomy in \autoref{fig:hw_taxonomy} to data elements that are
...
\begin{table}[]
    ...
    \begin{tabular}{|l|l|l|}
    \hline
    Data Element Type   & Reuse Type                    & Communication Type            \\ \hline
    Weights             & Temporal                      & Multicast w.r.t X \& Y loops  \\ \hline
    IFmap (Across Fmaps)& Temporal \& Spatial           & Multicast w.r.t F loop        \\ \hline
    IFmap (Within Fmaps)& Spatial (Due to KY/KX unroll) & Multicast w.r.t KY \& KX loops\\ \hline
    OFmap (Across Fmaps)& Temporal                      & Reduction w.r.t C loop        \\ \hline
    OFmap (Within Fmaps)& Spatial                       & Reduction w.r.t KY \& KX loops\\ \hline
    \end{tabular}
    \caption{Summary of Reuse and Communication Type for each Data Element in a convolution operation}
    \label{tab:summary_of_reuse_behavior}
\end{table}
